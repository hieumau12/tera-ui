---
title: Layout
description: TeraUI offers a set of layout options to customize the theme.
---

# Layout

TeraUI's plugin provides a variety of layout customization options. Alter spacing units,
...
`spacingUnit` token (default 4px), TeraUI auto-generates the following units:
...
TeraUI units behave like [Tailwind CSS spacing](https://tailwindcss.com/docs/customizing-spacing#default-spacing-scale) units. You can use them in the `margin`, `padding`,
...
TeraUI creates CSS variables using the format `--prefix-prop-name-scale` for each layout token. By
default the prefix is `teraui`, but you can change it with the `prefix` option.
...
/* With default prefix */
.my-button {
  padding: var(--teraui-spacing-unit-4);
  font-size: var(--teraui-font-size-small);
  line-height: var(--teraui-line-height-small);
  border-radius: var(--teraui-radius-medium);
}
/*  With custom prefix */
.my-component {
