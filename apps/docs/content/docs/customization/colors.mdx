---
title: Colors
description: Learn how to configure and customize colors with TeraUI.
---

# Colors

TeraUI's plugin enables you to personalize the semantic colors of the theme such as `primary`,
...
TeraUI offers a default color palette right out of the box, perfect for when you're
...
> Semantic colors should be placed inside the `teraui` plugin options, not inside the TailwindCSS theme object.
...
TeraUI creates CSS variables using the format `--prefix-colorname-shade` for each semantic color. By
...
/* With default prefix */
.my-component {
  background-color: var(--teraui-primary-500);
}
/*  With custom prefix */
.my-component {
