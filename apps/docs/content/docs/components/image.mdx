...
<ImportTabs
  commands={{
    main: 'import {Image} from "@tera-ui/react";',
    individual: 'import {Image} from "@tera-ui/image";',
  }}
/>
...
you can use it with TeraUI `Image` component as well.

<CodeDemo showPreview={false} title="With Next.js Image" files={imageContent.nextjs} />

> **Note**: TeraUI's `Image` component is `client-side`, using hooks like `useState` for loading states
