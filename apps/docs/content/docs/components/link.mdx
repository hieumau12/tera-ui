...
<ImportTabs
  commands={{
    main: 'import {Link} from "@tera-ui/react";',
    individual: 'import {Link} from "@tera-ui/link";',
  }}
/>
...
TeraUI components expose a `as` prop that allows you to customize the React element type that is used to render the component.
