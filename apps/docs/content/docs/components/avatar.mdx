...
TeraUI exports 3 avatar-related components:
...
<ImportTabs
  commands={{
    main: 'import {Avatar, AvatarGroup, AvatarIcon} from "@tera-ui/react";',
    individual: 'import {Avatar, AvatarGroup, AvatarIcon} from "@tera-ui/avatar";',
  }}
/>
...
TeraUI provides a `renderCount` prop to customize the count displayed when the `total` prop is passed.
