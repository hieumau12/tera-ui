...
export function extractNextUIImport(code: string): string[] {
  ...
  // Filter the lines to only include import statements from "@tera-ui/react"
  const importStatements = lines.filter(
    (line) => line.startsWith("import") && line.includes("@tera-ui/react"),
  );
...
export function updateTailwindConfig(tailwindConfig: string, componentNames: string[]): string {
  ...
  const lineIndex = lines.findIndex((line) =>
    line.includes("./node_modules/@tera-ui/theme/dist/**/*.{js,ts,jsx,tsx}"),
  );
  ...
  if (lineIndex !== -1) {
    ...
    componentNames.forEach((componentName) => {
      ...
      // Add the new line
      lines.splice(lineIndex, 0, `  "./node_modules/@tera-ui/theme/dist/components/${name}.js",`);
    });
  }
